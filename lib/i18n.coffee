...
Meteor.startup ->
  ...
  if Meteor.isServer
      ...
      while i < pathArray.length
        if pathArray[i].indexOf(".") is -1
          unless localI18n[pathArray[i]]
            ...
            addProperty localI18n, pathArray[i + 1]  if i + 1 < pathArray.length
        i++
      callback and callback(localI18n)
    ...
    addProperty = (obj, property, callback) ->
      ...
      callback and callback(obj[property])

    
    ###
    @function
    @name removeTrailingSlash
    @description Removes trailing Slash from string if its exsists
    ...
    i18n.get = (locale, param) ->
      locale = locale or SiC.get('language')
  ...
  if Meteor.isClient
    ...
    @description Set new locale if it is configured in /i18n/i18n.json config file.
    Update session's and localStorage or cookie (via SiC.storage) dependencies
    ...
    i18n.init = (defaultLocale) ->
      if i18n.isReady and not i18n.isStarted
        ...
        SiC.storage.set "locale", (if (SiC.storage.get("locale")) then SiC.storage.get("locale") else (if (i18n.config[userLocale]) then userLocale else i18n.defaultLocale))
        i18n.isStarted = true
        i18n.setLocale SiC.storage.get("locale")
    ...
    throwError = (code, string) ->
      ...
      switch code
        when 404
          text = "No locale \"#{string}\" is definded in /i18n/i18n.json config file"
          description = "Check /i18n/i18n.json file for \"#{string}\" locale."
        else
          text = "Something is goes wrong in /client/lib/i18n-service.js"
          description = "Please check /client/lib/i18n-service.js for errors"
    ...
    i18n.get = (param, replacements) ->
      if replacements and Object::toString.call(replacements) is "[object Object]" and Object.keys(replacements.hash).length > 0 or replacements and Object::toString.call(replacements) is "[object String]"
    ...
    Meteor.subscribe "i18n", ->
      i18n.isReady = true
      i18n.init SiC.get('language') unless i18n.isStarted
